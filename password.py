...
<body>
    ...
    <script>
        ...
        function validatePassword() {
            ...
            // Corrección de escape para evitar SyntaxWarning en Python
            const isSym = /[\\W_]/.test(pwd);
...
def generar_tres_contrasenas(frases_input):
    # CAMBIO AQUÍ: Se eliminan por completo TODOS los espacios internos y externos de cada frase
    palabras = ["".join(p.split()) for p in frases_input.split(',')]
    palabras = [p for p in palabras if p][:3] 
